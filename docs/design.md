...
follow-up; it needs the zval ownership worked out first.
...
#[TemplateParameter] class
        │
        ▼
TemplateParser ──────────► TemplateDefinition (parameters + slots)
        │                    slots are own properties/parameters/returns that carry a parameter,
        │                    in either the placeholder or the attribute form
        ▼
TypeArgumentResolver ────► concrete type names
        │                    grammar, existence, bounds, nested generics (eagerly, innermost first)
        ▼
AngleBracketNameMangler ─► "App\Box<int>"
        │
        ▼
SubstitutionPlan ────────► TypeSubstitutionMap (by type name) + SlotSubstitutionMap (by slot)
        │                    strategies contribute rather than compete, so one template may mix forms
        ▼
Monomorphizer ───────────► ClassSpecializer::specialize()   ← the only call that touches the engine
```
