# Debug PHP build for the destructive test group.
#
# Copied from lisachenko/z-engine (tools/docker/php-debug.Dockerfile) rather than shared: the
# two repositories are versioned independently and a Dockerfile is not something Composer can
# install. If you change one, consider whether the other needs the same change.
#
# A --enable-debug build turns the silent memory corruption that engine surgery can cause into
# loud Zend assertion failures, and enables report_memleaks. That is exactly what the eviction
# tests need: they run destroy_zend_class() over a *copied* class entry while its template is
...
# Build locally with:
#   docker build -f tools/docker/php-debug.Dockerfile -t generics-php:debug .
ARG PHP_VERSION=8.4
